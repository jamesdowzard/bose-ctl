...
class BoseRFCOMM {
    ...
    /// Parse a BMAP string response from a given byte offset.
    private func parseString(_ resp: [UInt8], from offset: Int = 4) -> String {
        guard resp.count > offset else { return "" }
        return String(bytes: Array(resp[offset...]), encoding: .utf8)?
            .trimmingCharacters(in: CharacterSet(charactersIn: "\0")) ?? ""
    }

    /// Parse a list of 6-byte MACs from a BMAP response starting at a given offset.
    private func parseMacList(_ resp: [UInt8], countAt: Int, dataAt: Int) -> [[UInt8]] {
        guard resp.count > countAt else { return [] }
        let count = Int(resp[countAt])
        var macs: [[UInt8]] = []
        for i in 0..<count {
            let offset = dataAt + (i * 6)
            guard offset + 6 <= resp.count else { break }
            macs.append(Array(resp[offset..<(offset + 6)]))
        }
        return macs
    }
    ...
    func getAllState() -> HeadphoneState? {
        do {
            return try withRFCOMM { channel in
                ...
                func get(_ bytes: [UInt8], timeout: TimeInterval = 3.0) -> [UInt8]? {
                    guard let resp = sendBMAP(channel, bytes: bytes, timeout: timeout),
                          resp.count >= 5, resp[2] == OP_RESP else { return nil }
                    return resp
                }

                if let r = get([0x02, 0x02, OP_GET, 0x00]) {
                    s.batteryLevel = min(100, max(0, Int(r[4])))
                    s.batteryCharging = r.count >= 8 ? r[7] != 0 : false
                }
                if let r = get([0x1F, 0x03, OP_GET, 0x00]) { s.ancMode = Int(r[4]) }
                if let r = get([0x05, 0x05, OP_GET, 0x00]), r.count >= 6 {
                    s.volumeMax = Int(r[4]); s.volume = Int(r[5])
                }

                // Connected devices (audio ground truth)
                if let r = sendBMAP(channel, bytes: [0x05, 0x01, OP_GET, 0x00]),
                   r.count >= 7, r[0] == 0x05, r[1] == 0x01, r[2] == OP_RESP {
                    s.connectedDevices = parseMacList(r, countAt: 6, dataAt: 7)
                }

                // Per-device ACL status
                for (_, mac) in boseKnownDevices {
                    let cmd: [UInt8] = [0x04, 0x05, OP_GET, 0x06] + mac
                    if let r = sendBMAP(channel, bytes: cmd, timeout: 2.0),
                       r.count >= 11, r[2] == OP_RESP, (Int(r[10]) & 0x01) != 0 {
                        s.aclConnectedDevices.append(mac)
                    }
                }

                // String fields
                if let r = get([0x00, 0x05, OP_GET, 0x00]) { s.firmware = parseString(r) }
                if let r = get([0x00, 0x07, OP_GET, 0x00]) { s.serialNumber = parseString(r) }
                if let r = get([0x00, 0x0F, OP_GET, 0x00]) { s.productName = parseString(r) }
                if let r = get([0x12, 0x0D, OP_GET, 0x00]) { s.platform = parseString(r) }
                if let r = get([0x12, 0x0C, OP_GET, 0x00]) { s.codename = parseString(r) }
                if let r = get([0x01, 0x02, OP_GET, 0x00]), r.count >= 6 { s.deviceName = parseString(r, from: 5) }

                // Audio codec (try UTF-8, fall back to hex)
                if let r = get([0x05, 0x04, OP_GET, 0x00]) {
                ...
                // Scalar fields
                if let r = get([0x01, 0x0A, OP_GET, 0x00]) { s.multipointEnabled = r[4] != 0 }
                if let r = get([0x01, 0x0B, OP_GET, 0x00]) { s.autoOffTimer = Array(r[4...]) }
                if let r = get([0x01, 0x09, OP_GET, 0x00]) { s.immersionLevel = Array(r[4...]) }
                if let r = get([0x08, 0x07, OP_GET, 0x00]) { s.onHead = r[4] == 0x04 }

                // EQ
                if let r = sendBMAP(channel, bytes: [0x01, 0x07, OP_GET, 0x00]),
                   r.count >= 16, r[2] == OP_RESP {
                    s.eq = (
                        bass: Int(Int8(bitPattern: r[6])),
